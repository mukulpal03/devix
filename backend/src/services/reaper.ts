...
export class IdleContainerReaper {
  ...
  static start() {
    ...
    this.intervalId = setInterval(async () => {
      const idleProjectIds = ConnectionRegistry.getIdleProjects(CONTAINER_GRACE_PERIOD_MS);

      if (idleProjectIds.length === 0) return;

      console.log(`[Reaper] Found ${idleProjectIds.length} idle project(s) to evaluate.`);
      ...
      for (const projectId of idleProjectIds) {
        try {
          ...
          console.error(`[Reaper] Critical failure during cleanup of project "${projectId}":`, error);
        }
      }
    }, REAPER_INTERVAL_MS);
